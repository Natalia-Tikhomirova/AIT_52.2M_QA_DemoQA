...
public class BasePage {
    ...
    private void scrollToElement(WebElement element) {
        wait.until(ExpectedConditions.visibilityOf(element)); // ждём, когда элемент найдется
        js.executeScript("arguments[0].scrollIntoView(true);", element); // скроллим до него
        waitForPageScrollToFinish(); // проверяем остановился скрол или нет
        wait.until(ExpectedConditions.elementToBeClickable(element)); // ждём, когда элемент станет кликабельным
    }
